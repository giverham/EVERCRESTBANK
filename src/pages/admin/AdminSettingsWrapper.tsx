...
export function AdminSettingsWrapper() {
  const [activeTab, setActiveTab] = useState<'general' | 'statements'>('general');

  return (
    <div className="space-y-6 max-w-5xl">
      {/* Tabs */}
      <div className="flex items-center gap-1 bg-secondary-100 dark:bg-secondary-800 p-1 rounded-xl w-fit">
        <button
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === 'general' ? <AdminSettingsPage /> : <StatementSettingsTab />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
