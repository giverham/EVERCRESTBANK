...
import {
  ArrowUpRight, ArrowDownRight, Send, Receipt, Download, FileText, Calendar,
} from 'lucide-react';
import { Card, SectionHeading } from '../../components/ui/Card';
import { Badge } from '../../components/ui/Badge';
import {
  demoAccounts, demoTransactions, demoUpcomingPayments,
  monthlySpending, spendingByCategory, formatCurrency,
} from '../../data/demoData';
...
const quickActions = [
  { label: 'Transfer', icon: Send, to: '/dashboard/transactions' },
  { label: 'Pay Bills', icon: Receipt, to: '/dashboard/transactions' },
  { label: 'Deposit', icon: Download, to: '/dashboard/accounts' },
  { label: 'Statements', icon: FileText, to: '/dashboard/statements' },
];

const maxSpend = Math.max(...monthlySpending.map((m) => m.amount));
const maxCat = Math.max(...spendingByCategory.map((c) => c.amount));
        ...
        {demoAccounts.map((acc, i) => (
          <motion.div key={acc.id} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.1 }}>
            <Card className="p-6 h-full">
              ...
              <p className="text-3xl font-bold text-primary-900 dark:text-white">{formatCurrency(acc.availableBalance)}</p>
              ...
              <div className="mt-4 pt-4 border-t border-secondary-200 dark:border-secondary-800 space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-secondary-500 dark:text-secondary-400">Current</span><span className="font-semibold text-primary-900 dark:text-white">{formatCurrency(acc.currentBalance)}</span></div>
                <div className="flex justify-between"><span className="text-secondary-500 dark:text-secondary-400">Pending</span><span className="font-semibold text-primary-900 dark:text-white">{formatCurrency(acc.pendingBalance)}</span></div>
                ...
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-secondary-100 dark:border-secondary-800 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-secondary-500 dark:text-secondary-400">Due {p.dueDate} · {p.category}</p>
                    </div>
                  </div>
